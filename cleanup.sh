...
run() {
  ...
  if [ "${DELETE_NAMESPACE}" -eq "1" ]; then
    ...
    echo "${oc_delete_tpl} && \\"
    oc_delete_tpl="oc get cm -n $NAMESPACE -o name | grep \"consumer-\|producer-\" | xargs -I % -n 1 oc delete % -n ${NAMESPACE}"
    echo "${oc_delete_tpl} && \\"
    oc_delete_tpl="oc get pvc -n $NAMESPACE -o name | grep -i readiness | xargs -I % -n 1 oc delete % -n ${NAMESPACE}"
    echo "${oc_delete_tpl} && \\"
